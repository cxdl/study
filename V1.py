...
chatbot1 = main(configure("config1.json"))
chatbot2 = main(configure("config2.json"))
chatbot3 = main(configure("config3.json"))
...
def chatPrint(chatbot, user_text):
    prev_text = ""
    for data in chatbot.ask(user_text):
...
def chat(user_text):
    ...
    for i in range(3):
        try:
            chatPrint(chatbot, user_text)
            break
        except:
            chatbot = getChatbot()
            print(f'sorry,网络波动，retry {i+1}')
    else:
        print('sorry,网络波动，请再试一次或者等待一会')
        sio.emit('response', 'sorry,网络波动，请再试一次或者等待一会')


if __name__ == "__main__":
    sio.run(app, host="10.90.167.99", port=5000, debug=True)
